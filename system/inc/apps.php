...
// echo "apps_hash sec: ". $_SESSION['apps_hash'] ."<br />";
// echo "apps_hash cok: ". $_COOKIE['apps_hash'] ."<br />";
...
if($_GET['act'] == 'upload'){
	...
	$type = explode(".", $image_name); // формат файла
	$type = end($type); // формат файла
		...
		if(in_array(strtolower($type), $allowed_files)){
			...
			if(move_uploaded_file($image_tmp, $rImg)){
					...
					if($rowCheck){
						...
						//Читаем хеш
						$app_hash = $db->safesql( ($_SESSION['apps_hash'] ? $_SESSION['apps_hash'] : $_COOKIE['apps_hash']) );
...
if(isset($_POST['send'])){
	...
	$swf_url = textFilter($_POST['swf_url']);
	...
	if(isset($title) AND !empty($title)){
		...
		if($sql_){
			...
			$swf_ava = "";
			foreach($sql_ as $row){
				if($row['type'] == 'poster') { $poster_ava = $row['file']; }
				if($row['type'] == 'swf') { $swf_ava = $row['file']; }
			}
			...
			if ($poster_ava && $swf_ava) {
				...
				$db->query("INSERT INTO `".PREFIX."_games` SET title = '{$title}', descr = '{$descr}', poster = '{$poster}', flash = '{$swf}', swf_url = '{$swf_url}', date = '{$server_time}', width = '{$width}', height = '{$height}'");
				$id = $db->insert_id();
				...
				//Обновляем ИД в таблице _games_files
				...
				unset($_COOKIE['apps_hash']);
				
			} else {
				msgbox('Ошибка', (!$poster_ava ? "Отсутствует Постер 75х75<br />" : "") . (!$swf_ava ? "Отсутствует Flash игра .swf<br />" : "") .'Заполните все поля!', '?mod=apps');
			}
			
		} else {
			msgbox('Ошибка', 'Отсутствует Постер 75х75<br />Отсутствует Flash игра .swf<br />Заполните все поля!!', '?mod=apps');
		} // db sql_
	} else {
		msgbox('Ошибка', 'Отсутствует Название<br />Заполните все поля!!', '?mod=apps');
	} // empty title
...
if(isset($_POST['edtisave'])){
	...
	if(isset($title) AND !empty($title)){
		...
		$db->query("UPDATE `".PREFIX."_games` SET title = '{$title}', descr = '{$descr}', swf_url = '{$swf_url}', width = '{$width}', height = '{$height}' WHERE id = '{$id}'");
...
if($_GET['act'] == 'edit'){
	...
	$row = $db->super_query("SELECT id, title, descr, flash, swf_url, poster, width, height FROM `".PREFIX."_games` WHERE id = '{$id}'");
	...
	if($row){
		...
		if($sql_scr){
			...
			foreach($sql_scr as $row_scr){
				...
				$imgs .= <<<HTML
<div style="position:relative;display:inline-block;vertical-align:top;margin:5px;">
	<a id="{$exps[0]}" href="#" onClick="del('{$row_scr['file']}'); return false;" style="position:absolute;top:5px;right:5px;cursor:pointer;display: inline-block;padding: 3px; background: #fff;" title="Удалить">[x]</a>
	<img  src="/uploads/apps/{$id}/m{$row_scr['file']}" alt="" style="width: 140px;" />
</div>
HTML;
	...
	aj3 = new AjaxUpload('upload_3', {
		...
		onComplete: function (file, row){
			if(row == 1){
				...
				file = row.split('/m');
				fid = file[1].split('.');
				$('#scrins').append('<div style="position:relative;display:inline-block;vertical-align:top;margin:5px;"><a id="'+fid[0]+'" href="#" onClick="del(\''+file[1]+'\'); return false;" style="position:absolute;top:5px;right:5px;cursor:pointer;display: inline-block;padding: 3px; background: #fff;" title="Удалить">[x]</a><img src="/uploads/apps/'+row+'" alt="" style="width: 140px;" /></div>');
			}
			$('#upload_3').show();
			$('#prog_scrins').hide();
		}
...
<div class="fllogall" style="width:180px">Название:<span style="color:red;">*</span></div>
...
<div class="fllogall" style="width:180px">Постер 75х75:<span style="color:red;">*</span></div>
...
if (  empty($row['swf_url']) ) {
echo <<<HTML
<div class="fllogall" style="width:180px">Flash игра .swf:<span style="color:red;">*</span></div>
 <input type="submit" value="Выбрать файл" class="inp" id="upload_2" style="margin-top:0px" /><br />
 <label>http://lezgi.net/uploads/apps/{$row['id']}/{$row['flash']}</label>
...
echo <<<HTML
<div class="fllogall" style="width:180px">SWF url:</div>
 <input type="text" name="swf_url" class="inpu" value="{$row['swf_url']}" />
<div class="mgcler"></div>
HTML;
}
echo <<<HTML
...
$(document).ready(function(){
	...
	$( "#swf_load_type" ).change(function() {
		var this_val = $( this ).val();
		if (this_val==1) {
			$( ".swf_load_type1" ).css("display", "block");
			$( ".swf_load_type2" ).css("display", "none");
		} else {
			$( ".swf_load_type2" ).css("display", "block");
			$( ".swf_load_type1" ).css("display", "none");
		}
	});
	...
	aj3 = new AjaxUpload('upload_3', {
		...
		onComplete: function (file, row){
			if(row == 1){
				...
				file = row.split('/m');
				fid = file[1].split('.');
				$('#scrins').append('<div style="position:relative;display:inline-block;vertical-align:top;margin:5px;"><a id="'+fid[0]+'" href="#" onClick="del(\''+file[1]+'\'); return false;" style="position:absolute;top:5px;right:5px;cursor:pointer;display: inline-block;padding: 3px; background: #fff;" title="Удалить">[x]</a><img src="/uploads/apps/temp/'+row+'" alt="" /></div>');
			}
			$('#upload_3').show();
			$('#prog_scrins').hide();
		}
...
<div class="fllogall" style="width:180px">Тип загрузки SWF файла:</div>
 <select name="swf_load_type" id="swf_load_type" size="1" class="inpu" style="width:200px;">
 	<option value="1">Загрузка</option>
 	<option value="2">Сcылка</option>
 </select>
<div class="mgcler"></div>

<div class="swf_load_type1">
	<div class="fllogall" style="width:180px">Flash игра .swf:<span style="color:red;">*</span></div>
	 <input type="submit" value="Выбрать файл" class="inp" id="upload_2" style="margin-top:0px" /><br />
	 <div id="prog_flash" style="display:none;margin-top:-11px;background:url('/system/inc/images/progress_grad.gif');width:94px;height:18px;border:1px solid #006699;margin-left:182px"></div>
	 <div style="margin-left:180px"><small id="ok_swf">Файл не должен превышать 100 Mб.</small></div>
...
<div class="swf_load_type2" style="display:none;">
	<div class="fllogall" style="width:180px">SWF url:</div>
	 <input type="text" name="swf_url" class="inpu" value="" />
	<div class="mgcler"></div>
</div>

<div class="fllogall" style="width:180px">Ширина flash игры (px):</div>
 <input type="text" name="width" class="inpu" value="642" />
<div class="mgcler"></div>

<div class="fllogall" style="width:180px">Высота flash игры (px):</div>
 <input type="text" name="height" class="inpu" value="482" />
<div class="mgcler"></div>
