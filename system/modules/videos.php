	...
	switch($act){
		...
		case "send":
			...
			if($config['video_mod_add'] == 'yes'){
				$good_video_lnk = ajax_utf8(textFilter( trim(strip_tags($_POST['good_video_lnk']))));

				######## youtu.be #######
				if ( preg_match("/youtu.be/", $good_video_lnk) ) {
					$good_video_lnk = explode('youtu.be/', $good_video_lnk);
					$good_video_lnk = explode('&', $good_video_lnk[1]);
					$good_video_lnk = "https://www.youtube.com/watch?v=". $good_video_lnk[0];
				}
				######## //youtu.be #######
				...
				if(preg_match("/src=\"https:\/\/www.youtube.com|src=\"https:\/\/youtube.com|src=\"http:\/\/www.youtube.com|src=\"http:\/\/youtube.com/i", 'src="'.$good_video_lnk)){
					...
					$result_video_lnk = 'http://www.youtube.com/embed/'.$exp_x[0].'';
				}
				
				//Если rutube, То добавляем префикс value="
				if(preg_match("/value=\"https:\/\/www.rutube.ru|value=\"https:\/\/rutube.ru|value=\"http:\/\/www.rutube.ru|value=\"http:\/\/rutube.ru/i", 'value="'.$good_video_lnk)){
					
					// $exp_frutube = explode('?v=', $good_video_lnk);


					$url = trim(strip_tags($good_video_lnk));
					$ch = curl_init();
					...
					preg_match_all('`(<meta property="og:video:iframe" content="[^\[]+\<meta property="og:restrictions:age")`si', $data_rutube, $parse_frutube);
					$exp_frutube = rn_replace(str_replace(array('<meta property="og:video:iframe" content="', '<meta property="og:restrictions:age"', '" />'), '', $parse_frutube[1][0]));
					...
					// $result_video_lnk = '<iframe width="770" height="420" src="'. $exp_frutube .'" frameborder="0" allowfullscreen></iframe>';
					$result_video_lnk = ''. $exp_frutube .'';
				}
				
				//Если vimeo, То добавляем префикс src="
				if(preg_match("/src=\"https:\/\/www.vimeo.com|src=\"https:\/\/vimeo.com|src=\"http:\/\/www.vimeo.com|src=\"http:\/\/vimeo.com/i", 'src="'.$good_video_lnk)){
					...
					$result_video_lnk = 'http://player.vimeo.com/video/'.$exp_frutube[1].'';
				}
				...
				if(preg_match("/src=\"http:\/\/www.smotri.com|src=\"http:\/\/smotri.com/i", 'src="'.$good_video_lnk)){
					...
					// $result_video_lnk = '<iframe src="http://pics.smotri.com/player.swf?file='.$exp[1].'" width="770" height="420" frameborder="0"></iframe>';
					$result_video_lnk = 'http://pics.smotri.com/player.swf?file='.$exp[1].'';
				}
				
				//Формируем данные о фото
				$photo = $db->safesql(ajax_utf8(htmlspecialchars(trim(strip_tags($_POST['photo'])))));
				...
				//Разришенные форматы
				$allowed_files = array('jpg', 'jpeg', 'jpe', 'png', 'gif', 'webp');

				//Загружаем картинку на сайт
				if(in_array(strtolower($img_format), $allowed_files) && $result_video_lnk){
		...
		case "load":
			...
			$video_lnk = trim(strip_tags($_POST['video_lnk']));

			######## youtu.be #######
			if ( preg_match("/youtu.be/", $video_lnk) ) {
				$video_lnk = explode('youtu.be/', $video_lnk);
				$video_lnk = explode('&', $video_lnk[1]);
				$video_lnk = "https://www.youtube.com/watch?v=". $video_lnk[0];
			}
			######## //youtu.be #######
			
			if(preg_match("/https:\/\/www.youtube.com|https:\/\/youtube.com|http:\/\/www.youtube.com|http:\/\/youtube.com|https:\/\/rutube.ru|https:\/\/www.rutube.ru|http:\/\/rutube.ru|http:\/\/www.rutube.ru|https:\/\/www.vimeo.com|https:\/\/vimeo.com|http:\/\/www.vimeo.com|http:\/\/vimeo.com|http:\/\/smotri.com|http:\/\/www.smotri.com/i", $video_lnk)){
			
				//Открываем ссылку
				
				//Если ссылка youtube, то формируем xml ссылку для получения данных
				if(preg_match("/https:\/\/www.youtube.com|https:\/\/youtube.com|http:\/\/www.youtube.com|http:\/\/youtube.com/i", $video_lnk)){
				...
				if(!$sock){
					...
					if(preg_match("/https:\/\/www.youtube.com|https:\/\/youtube.com|http:\/\/www.youtube.com|http:\/\/youtube.com|https:\/\/rutube.ru|https:\/\/www.rutube.ru|http:\/\/rutube.ru|http:\/\/www.rutube.ru|https:\/\/smotri.com|https:\/\/www.smotri.com|http:\/\/smotri.com|http:\/\/www.smotri.com|https:\/\/www.vimeo.com|https:\/\/vimeo.com|http:\/\/www.vimeo.com|http:\/\/vimeo.com/i", $video_lnk)){
					...
					// if(preg_match("/https:\/\/www.vimeo.com|https:\/\/vimeo.com|http:\/\/www.vimeo.com|http:\/\/vimeo.com/i", $video_lnk)){
					// 	while(!feof($sock)){
					...
					if(preg_match("/https:\/\/www.youtube.com|https:\/\/youtube.com|http:\/\/www.youtube.com|http:\/\/youtube.com/i", $video_lnk)){
						...
						preg_match_all('`(<meta property="og:title" content="[^\[]+\<meta property="og:image")`si', $data_youtube, $parse_youtube);
						$res_title = rn_replace(str_replace(array('<meta property="og:title" content="', '<meta property="og:image"', '" />'), '', $parse_youtube[1][0]));
						$res_title = str_replace('">', "", $res_title);

						preg_match_all('`(<meta name="description" content="[^\[]+\<meta name="keywords")`si', $data_youtube, $parse_youtube_descr);
						$res_descr = rn_replace(str_replace(array('<meta name="description" content="', '<meta name="keywords"', '" />'), '', $parse_youtube_descr[1][0]));
					...
					if(preg_match("/https:\/\/rutube.ru|https:\/\/www.rutube.ru|http:\/\/rutube.ru|http:\/\/www.rutube.ru/i", $video_lnk)){
						...
						preg_match_all('`(<meta property="og:image" content="[^\[]+\<meta property="og:video")`si', $data_rutube, $parse_img);
						$res_img = rn_replace(str_replace(array('<meta property="og:image" content="', '<meta property="og:video"', '" />'), '', $parse_img[1][0]));
						$res_img = str_replace('">', "", $res_img);
						$res_img = "http:". $res_img ."";
					}
					...
					if(preg_match("/https:\/\/www.vimeo.com|https:\/\/vimeo.com|http:\/\/www.vimeo.com|http:\/\/vimeo.com/i", $video_lnk)){
						...
						preg_match_all('`(<meta property="og:title" content="[^\[]+\<meta property="og:description")`si', $data_vimeo, $parse_vimeo);
						$res_title = rn_replace(str_replace(array('<meta property="og:title" content="', '<meta property="og:description"', '" />'), '', $parse_vimeo[1][0]));
						$res_title = str_replace('">', "", $res_title);
						...
						preg_match_all('`(<meta property="og:image" content="[^\[]+\<meta property="og:image:secure_url")`si', $data_vimeo, $parse_img);
						$res_img = rn_replace(str_replace(array('<meta property="og:image" content="', '<meta property="og:image:secure_url"', '" />'), '', $parse_img[1][0]));
						$res_img = str_replace('">', "", $res_img);
						$res_img = str_replace(".webp", ".jpg", $res_img);
						...
						preg_match_all('`(<meta property="og:description" content="[^\[]+\<meta property="og:updated_time")`si', $data_vimeo, $parse_vimeo_descr);
						$res_descr = rn_replace(str_replace(array('<meta property="og:description" content="', '<meta property="og:updated_time"', '" />'), '', $parse_vimeo_descr[1][0]));
						$res_descr = str_replace('">', "", $res_descr);
					}
					
					//Если видеосервис smotri
					if(preg_match("/https:\/\/smotri.com|https:\/\/www.smotri.com|http:\/\/smotri.com|http:\/\/www.smotri.com/i", $video_lnk)){
						...
						preg_match_all('`(<meta property="og:title" content="[^\[]+\<meta property="og:description")`si', $data_smotri, $parse_smotri);
						$res_title = rn_replace(str_replace(array('<meta property="og:title" content="', '<meta property="og:description"', '" />'), '', $parse_smotri[1][0]));
						$res_title = str_replace('">', "", $res_title);


						preg_match_all('`(<meta property="og:description" content="[^\[]+\<meta property="og:image")`si', $data_smotri, $parse_smotri_descr);
						$res_descr = rn_replace(str_replace(array('<meta property="og:description" content="', '<meta property="og:image"', '" />'), '', $parse_smotri_descr[1][0]));
						$res_descr = str_replace('">', "", $res_descr);

						preg_match_all('`(<meta property="og:image" content="[^\[]+\<meta property="og:url")`si', $data_smotri, $parse_img);
						$res_img = rn_replace(str_replace(array('<meta property="og:image" content="', '<meta property="og:url"', '" />'), '', $parse_img[1][0]));
						$res_img = str_replace('">', "", $res_img);
						$res_img = trim($res_img);
						$res_img = "http:". $res_img ."";
					}
		...
		case "page":
			...
			if ($get_user_id) {
				...
				if($last_id) {
						...
						//SQL Запрос
						$sql_ = $db->super_query("SELECT id, title, photo, comm_num, add_date, SUBSTRING(descr, 1, 180) AS descr FROM `".PREFIX."_videos` WHERE id < '{$last_id}' ORDER by `add_date` DESC LIMIT 0, {$limit_vieos}", 1);
						...
						if($sql_){
							...
							foreach($sql_ as $row){
								...
								if ($row['owner_user_id']!=$user_id) { $CheckBlackList = CheckBlackList($row['owner_user_id']); }
								if (!$CheckBlackList) {
									if ($row['owner_user_id']==$user_id) {
										$privacy_arr = array(0 => '1', 1 => '2', 2 => '3');
									} else {
										$privacy_arr = array(0 => '1');
									}
									if ($row['owner_user_id']!=$user_id) { $myCheckFriends = CheckFriends($row['owner_user_id']); }
									if ($myCheckFriends) {
										$privacy_arr = array(0 => '1', 1 => '2');
									}

									if ( in_array($row['privacy'], $privacy_arr) ) {
		...
		if($get_user_id) {
			...
			//SQL Запрос
			$sql_ = $db->super_query("SELECT id, title, photo, comm_num, add_date, privacy, owner_user_id, SUBSTRING(descr, 1, 180) AS descr FROM `".PREFIX."_videos` ORDER by `add_date` DESC LIMIT 0, {$limit_vieos}", 1);
			...
			if($sql_) {
				...
				foreach ($sql_ as $row) {
					...
					// if ( CheckBlackList() ) {
					// 	echo "Yest CheckBlackList<br />";
					// } else {
					// 	echo "Net CheckBlackList<br />";
					// }

					// if ( CheckFriends() ) {
					// 	echo "Yest CheckFriends<br />";
					// } else {
					// 	echo "Net CheckFriends<br />";
					// }
