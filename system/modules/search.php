...
if($logged){
	...
	$tpl->set('{query-people}', '/type1'. ($_GET['query'] ? '/query='. $_GET['query'] : ""));
	// $tpl->set('{query-videos}', '&type=2&query='.$_GET['query']);
	$tpl->set('{query-videos}', '/type2'. ($_GET['query'] ? '/query='. $_GET['query'] : ""));
	// $tpl->set('{query-notes}', '&type=3&query='.$_GET['query']);
	$tpl->set('{query-notes}', '/type3'. ($_GET['query'] ? '/query='. $_GET['query'] : ""));
	// $tpl->set('{query-groups}', '&type=4&query='.$_GET['query']);
	$tpl->set('{query-groups}', '/type4'. ($_GET['query'] ? '/query='. $_GET['query'] : ""));
	// $tpl->set('{query-audios}', '&type=5&query='.$_GET['query']);
	$tpl->set('{query-audios}', '/type5'. ($_GET['query'] ? '/query='. $_GET['query'] : ""));
	...
	if($sql_){
		...
		//Если критерий поиск "по сообещствам"
			...
			foreach($sql_ as $row){
				...
				else $tpl->set('{adres}', 'groups/'.$row['id']);
				$tpl->compile('content');
			}
