...
if($logged){
	...
	if($row['del'] == 1){
		...
		if($row['traf']){
			$tpl->set('{num-2}', '<a href="/publics/'.$row['id'].'" onClick="groups.all_people(\''.$row['id'].'\'); return false">'.gram_record($row['traf'], 'subscribers2').'</a>');
			$tpl->set('{no-users}', '');
		} else {
		...
		if($row['videos_num']){
			...
			foreach($sql_videos as $row_video){
				...
				$videos .= "
<div class=\"profile_one_video\"><a href=\"/videos/{$row_video['owner_user_id']}/{$row_video['id']}\" onClick=\"videos.show({$row_video['id']}, this.href, '/{$row['adres']}'); return false\"><img src=\"{$row_video['photo']}\" alt=\"\" width=\"185\" /></a><div class=\"video_profile_title\"><a href=\"/videos/{$row_video['owner_user_id']}/{$row_video['id']}\" onClick=\"videos.show({$row_video['id']}, this.href, '/{$row['adres']}'); return false\">{$row_video['title']}</a></div><div class=\"nesubscriptstatus\">{$date_video} - <a href=\"/videos/{$row_video['owner_user_id']}/{$row_video['id']}\" onClick=\"videos.show({$row_video['id']}, this.href, '/{$row['adres']}'); return false\">{$comm_num}</a></div></div>
				";
