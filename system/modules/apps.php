...
if($logged){
	...
	switch($act){
		...
		case "start":
			...
			//Выводим данные о игре
			$row = $db->super_query("SELECT title, poster, flash, swf_url, traf, width, height FROM `".PREFIX."_games` WHERE id = '{$id}'");
			...
			if($row){
				...
				$tpl->set('{title}', stripslashes($row['title']));
				$tpl->set('{swf}', ($row['swf_url'] ? $row['swf_url'] : "/uploads/apps/{$id}/{$row['flash']}"));
				$tpl->set('{app-id}', $id);
			...
			if(!$_POST['doload'] OR $_POST['doload'] == 2){
				...
				if($sqlFRlist){
					...
					if($sql_acti){
						...
						if(!$_POST['doload'])
							$tpl->result['activity'] = '<div class="info_center" style="width:299px;padding-top:125px;padding-bottom:100px">Здесь будет отображаться активность<br /> Ваших друзей в разделе игр.</div>';
