...
if($row){
	...
	$metatags['title'] = strip_tags($row['title']);
	$metatags['description'] = strip_tags($row['descr']);
	...
	if($privacy){
		...
		$tpl->load_template('videos/full.tpl');
			...
			//Видео страница или нет
			if(isset($_GET['get_video_id'])){
				$tpl->set('[get_video_page]', '');
				$tpl->set('[/get_video_page]', '');
				$tpl->set_block("'\\[not-get_video_page\\](.*?)\\[/not-get_video_page\\]'si","");
			} else {
				$tpl->set('[not-get_video_page]', '');
				$tpl->set('[/not-get_video_page]', '');
				$tpl->set_block("'\\[get_video_page\\](.*?)\\[/get_video_page\\]'si","");
			}
		...
		$tpl->set('{go_to_link}', (isset($_GET['get_video_id']) ? true : false));
