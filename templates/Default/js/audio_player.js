...
var player = {
	open: function(type, uid){
		...
		if(ln){
				...
				if(tPos == 0)
					$('.staticPlbg').animate({'margin-top': -600});
				else
					$('.staticPlbg').animate({'margin-top': 0});
			} else {
				...
				jQpUserId = uid;
				$.post('/index.php?go=audio_player', {get_user_id: uid}, function(d){
					if (d!="no_music_content") {
						$('#audioPlayer').html('<div class="staticPlbg azad" id="staticPlbg" style="margin-left:'+pos+'px">'+d+'</div>');
						if(type) {
							player.change_list(uid);
						}
					}
				});
			}
	...
	next: function(){
		...
		//DO LOAD AUDIOS
		var allNum = size - 20; // MAx Load Size
	...
	$.post('/index.php?go=audio_player', {query: a, doload: 1, get_user_id: jQpUserId}, function(d){
		...
		$('.staticpl_audios').css('display', 'block');
		$('.music_none_content').css('display', 'none');
	...
	$('.staticPlbg').removeClass('page_audio').css('margin', '20px').css('margin-top', '0').css('margin-left', pos+'px').css('margin-top', '-600px');
