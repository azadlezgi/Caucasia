...
var videos = {
	...
	serviece: function(request){
		var pattern = new RegExp(/https:\/\/www.youtu.be|https:\/\/youtu.be|https:\/\/www.youtube.com|https:\/\/youtube.com|http:\/\/www.youtube.com|http:\/\/youtube.com|https:\/\/rutube.ru|https:\/\/www.rutube.ru|http:\/\/rutube.ru|http:\/\/www.rutube.ru|https:\/\/www.vimeo.com|https:\/\/vimeo.com|http:\/\/www.vimeo.com|http:\/\/vimeo.com|https:\/\/smotri.com|https:\/\/www.smotri.com|http:\/\/smotri.com|http:\/\/www.smotri.com/i);
	...
	prev: function(req_href){
		...
		video_url = '/videos/'+filter_two[1]+'/'+filter_one[1];
		videos.show(filter_one[1], video_url);
	},
	close: function(owner_id, close_link, go_to_link){
		$('.video_view').remove();
		$('html, body').css('overflow-y', 'auto');
		...
		if (go_to_link) {
			Page.Go('/videos/'+ owner_id);
			// console.log('go_to_link: ', go_to_link);
		} else {
			if(close_link) {
				history.pushState({link: close_link}, null, close_link);
			} else {
	...
	setEvent: function(event, owner_id, close_link, go_to_link){
		var oi = (event.target) ? event.target.id: ((event.srcElement) ? event.srcElement.id : null);
		var el = oi.substring(0, 10);
		if(el == 'video_show')
			videos.close(owner_id, close_link, go_to_link);
	},
...
'</div>';
	...
	wall_video_add_select: function(){
		var video_attach_lnk = $('#video_attach_lnk').val().replace('http://'+location.host+'/videos/', '');
		var data = video_attach_lnk.split('/');
...
var reviews = {
  send: function(){
	...
	if(text != 0){
	  ...
	  $('#ok_send').hide();
	  $.post('/index.php?go=reviews&act=send', {text: text}, function(){
	    $('#ok_send').show();
	    $('#text').val('');
